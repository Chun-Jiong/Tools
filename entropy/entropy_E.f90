...
PROGRAM main
	...
	do iblck = 1, NBlck
		...
		do i = 2, NObs
			entropy(i,iblck) = entropy(i-1,iblck) - Obs(i-1,iblck)/(temperature(i)**2)*abs(temperature(i)-temperature(i-1))
		end do 
		do i = 2, NObs
			entropy(i,iblck) = entropy(i,iblck) + Obs(i,iblck)/temperature(i)
		end do
	end do 
	Obs = entropy
	deallocate(entropy)
